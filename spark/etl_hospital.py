...
⚠️ legacy 版本（2026-09-15 起由 spark/jobs/ 取代，保留作对照与回溯）
--------------------------------------------------------------------
现行链路见 spark/jobs/（run_all.py 编排）：
  · 存储：ODS/DWD/DWS 三层以 Parquet 落在 HDFS /hospital，仅 ADS 落 MySQL
  · 代码：按"分层 + 分析维度"拆分为 8 个模块（空间/类型等级/科室/网络/时间）
  · 依赖：spark-submit --packages 以 Maven 坐标声明，不再手工挂 --jars
本文件保留原因：① 论文可对照"单脚本 → 模块化"的重构过程；
             ② 万一需要回退到"全链路写 MySQL"的旧形态，可直接运行。
...
def main():
    ...
    inst_dwd = (
        ...
        # 市级重点专科数（北京市级临床重点专科 + 北京中医药重点专科；2026-09-06 联网采集）
