...
const routes = [
  // 默认入口 = 任务主线，而不是某个报表页。
  // 这个重定向就是「系统」与「大屏」的分界线：用户进来是来筛机构、要结果的，不是来看图的。
  { path: '/', redirect: '/filter' },
  { path: '/filter', name: 'filter', component: () => import('./views/SmartFilterView.vue'),
    meta: { title: '智能筛选' } },
...
// 未匹配的地址回主线，避免出现空白页
routes.push({ path: '/:pathMatch(.*)*', redirect: '/filter' })
