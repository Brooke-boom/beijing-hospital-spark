...
# ============================================================ 数据源 S4：北京市中医管理局 第二批"十四五"中医药重点专科（107 项目，2024/09 公示）
# 官方附件 .docx 解析（zyj.beijing.gov.cn/sy/tzgg/202409/t20240914_3891743.html）
# 由 /tmp/bjzy_docx/parse_batch2.py 生成 batch2_records.csv
S4_CSV = '/tmp/bjzy_docx/batch2_records.csv'

# ============================================================ 数据源 S5：北京市卫健委 2022 年度北京市重大疫情防治重点专科（29 项目）
# 北京市卫健委通知（2022-10-13 印发）"2022 年度北京市重大疫情防治重点专科项目名单"
# 来源：今日头条转载北京日报（北京市卫健委 2022-10 公示原文）
# 注：此批为公卫应急专项专科，与常规临床重点专科属不同项目但均属"市级专科"认定，统一入 municipal_specialty
S5 = [
    # === 培育类 9 项（限各区域医疗中心申报）===
    ('感染性疾病科', '培育', '北京市密云区医院'),
    ('感染性疾病科', '培育', '北京怀柔医院'),
    ...
    ('医学影像科',   '培育', '航天中心医院'),
    ('医学影像科',   '培育', '北京市延庆区医院'),
    ('重症医学科',   '培育', '首都医科大学附属复兴医院'),
    ('重症医学科',   '培育', '首都医科大学附属北京潞河医院'),
    ('中医科',       '培育', '首都医科大学附属复兴医院'),
    ('中医科',       '培育', '首都医科大学附属北京潞河医院'),
    # === 建设类 17 项 ===
    ('重症医学科',   '建设', '首都医科大学附属北京世纪坛医院'),
    ('重症医学科',   '建设', '首都医科大学附属北京地坛医院'),
    ('重症医学科',   '建设', '中日友好医院'),
    ('医学影像科',   '建设', '首都医科大学附属北京朝阳医院'),
    ('医学影像科',   '建设', '中国医学科学院阜外医院'),
    ('医学影像科',   '建设', '北京大学第三医院'),
    ('感染性疾病科', '建设', '北京清华长庚医院'),
    ('感染性疾病科', '建设', '首都医科大学附属北京儿童医院'),
    ('医学检验科',   '建设', '北京大学第一医院'),
    ('医学检验科',   '建设', '首都医科大学附属北京安贞医院'),
    ('呼吸内科',     '建设', '北京清华长庚医院'),
    ('呼吸内科',     '建设', '首都医科大学宣武医院'),
    ('流行病学',     '建设', '首都医科大学附属北京安贞医院'),
    ('流行病学',     '建设', '首都医科大学附属北京朝阳医院'),
    ('创伤科',       '建设', '北京大学人民医院'),
    ('创伤科',       '建设', '首都医科大学附属北京朝阳医院'),
    ('中医科',       '建设', '首都医科大学附属北京中医医院（感染危重病医学科）'),
    ('中医科',       '建设', '北京大学人民医院'),
    # === 卓越类 4 项（限已通过国家临床重点专科建设项目验收的三级甲等医院申报）===
    ('感染性疾病科', '卓越', '北京大学第一医院'),
    ('医学检验科',   '卓越', '北京大学人民医院'),
    ('中医科',       '卓越', '首都医科大学附属北京地坛医院'),
    ('中医科',       '卓越', '中国中医科学院西苑医院（肺病科）'),
]
...
# S4 第二批中医药（从 batch2_records.csv 自动读入）
import csv as _csv
if os.path.exists(S4_CSV):
    with open(S4_CSV, encoding='utf-8') as f:
        for row in _csv.DictReader(f):
            # 类别去尾（领超类 → 领超）
            tier = row['level'].replace('类', '')
            add(row['hospital'], row['spec'], batch=f'十四五-第二批-{tier}',
                program='北京市中医管理局中医药重点专科',
                source=f'北京市中医管理局 第二批"十四五"中医药重点专科（{row["code"]}）')
    print(f'[S4] 第二批中医药专科已读入 {S4_CSV}')
# S5 2022 重大疫情防治
for spec, tier, h in S5:
    add(h, spec, batch=f'2022-{tier}', program='北京市重大疫情防治重点专科',
        source='北京市卫健委 2022 年度北京市重大疫情防治重点专科项目')
...
report.append('- **S3**：北京市中医管理局 [首批"十四五"中医药重点专科名单](https://zyj.beijing.gov.cn/sy/tzgg/202211/t20221128_2867580.html)（74 项 — 2026-09-06 全部入库）')
report.append('- **S4**：北京市中医管理局 [第二批"十四五"中医药重点专科名单](https://zyj.beijing.gov.cn/sy/tzgg/202409/t20240914_3891743.html)（107 项 — 2026-09-06 全部入库，官方 docx 附件解析）')
report.append('- **S5**：北京市卫健委 2022 年度北京市重大疫情防治重点专科项目（29 项 — 培育/建设/卓越三档，公卫应急专项）')
report.append('')
report.append('## 关键统计\n')
...
report.append('')
report.append('## TODO\n')
report.append('- [x] 补齐 S3 中医十四五首批 74 项（2026-09-06 完成）')
report.append('- [x] 入库 S4 中医十四五第二批 107 项（2026-09-06 完成）')
report.append('- [x] 入库 S5 2022 年度北京市重大疫情防治重点专科 29 项（2026-09-06 完成）')
report.append('- [ ] 抓取 2024 年北京市临床重点专科西医批次（卫健委未发布完整公示，仅各医院新闻分散确认）')
