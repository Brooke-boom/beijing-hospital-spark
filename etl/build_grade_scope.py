...
背景：master 全表（2026-09-22 同名合并治理后 9,678 家）中仅约 874 家（9.0%）属于
参加医院等级评审的医疗机构，
...
# ⚠️ 其中这六类要**提到「level 非空」之前判定**（2026-09-22 修正）。
#    它们按各自《基本标准》以「诊疗科目」核准执业，本就没有等级建制，
#    出现的 level 只可能是脏值或文件名误解析：
#      · 《大兴区一级以下医院名单…诊所名单、医务室名单》 → 313 家盖上「一级」
#      · 《丰台区村一级卫生室》 → 16 家村卫生室被读成「一级」
#    原顺序让「level 非空 → applicable」短路在前，这 334 家反被判成"参评"，
#    于是在 03 段矩阵里以「一级」占位 —— 大兴区一度出现 369 家一级机构。
#    社区卫生服务中心**不在**本次收紧范围：北京多由原街道医院转制、确有等级。
NOT_RATED_CAT = ('诊所', '村卫生室', '门诊部', '社区卫生服务站', '医务室', '护理站')
...
def decide(r):
    ...
    # 顺序修正（2026-09-22）：六类无等级建制的机构必须先判，见 NOT_RATED_CAT 注释。
    if any(k in cat for k in NOT_RATED_CAT):
        return 'not_applicable', f'类别={cat} 不设等级建制'
    if level:
