...
- [x] **大屏响应式版面美化（2026-09-21）**：四个页面「看着杂乱」的观感问题逐条量化收口——
  ...
  - 新增闸门 **`etl/verify_layout.py`（68 项断言）**：Playwright 显式视口 × 5 档宽度（1440/1280/1200/1080/900）× 4 个页面，量横向溢出、内容逃逸（`overflow:visible` 却比容器大）、行内参差、末行填充率、每行最少格数、行内等宽、整合页两栏齐平、质量页 2×2、标题字号字重与分隔线。反向测试 R1（面板恢复定高）/ R2（网格退化成一格一行）/ R3（删掉窄屏定位重置）三个样本全部被捕获。
