...
**智能导诊（疾病/症状 → 科室 → 医院）**：基于本地知识库 `dim_disease_dept`（266 条常见病/症状 → 29 个标准科室的映射，覆盖 33 条急诊条目），输入自然语言病情（如"头痛""胸痛""儿童发烧"）即匹配对应科室，再联 `dwd_dept_relation_clean` 筛出具备该科室的医院，复用条件匹配度加权排出 Top N 并标注急诊优先。全程**离线零依赖**，不依赖大模型，断网可跑；维度表缺失时回退到 `data/processed/disease_dept_map.csv`。
...
**可视化系统：七视图导航（信息架构对齐毕业设计题目）**：前端通过左侧导航栏切换七个视图，浅色/深色统一由设计令牌驱动：

| 视图 | 对应论文 | 内容 |
|---|---|---|
| 🏠 数据总览 | 系统总体展示 | 4 张 KPI（机构总数 / 医院 732 / 社区机构 / 数据源 70）+ 北京 16 区分级地图（点击区名联动筛选）+ 类型 / 等级 / 办别图 + 数据来源与质量概览条 |
| 📊 医疗资源分析 | 数据分析 | 四项 KPI + **八维分析图表**，全部随筛选实时联动 |
| 🏥 机构查询 | 数据应用 | 七维筛选 + 机构列表（勾选对比 / 收藏 / 分页）+ 机构详情抽屉 |
| 🔎 智能筛选 ⭐ | 核心功能 | 自然语言筛选（**基于规则解析的条件转换**，大模型仅零命中兜底）→「已识别 N 个条件」chips + 排序单选（条件匹配度 / 距离 / 等级 / 科室 / 名称）+ Top 5 预览 → 一键跳转机构查询完整结果 |
| 🗂 数据整合 ⭐ | Spark 核心技术 | 三类数据源（医保局 4,876 / 社区 1,972 / 区级与专题 6,955，合计 13,803 条）+ Spark 八步处理流程时间线 + 处理结果（70 文件 → 9,791 家）+ ETL 批次时效表 |
| 📈 数据质量 | 数据治理 | 真实治理口径：8 个关键字段完整率（含进度条）+ 坐标精度分布 + 去重与多源交叉验证（重复名 3,139 / ≥3 源验证 657 家）+ 清洗后类别构成 |
| ℹ️ 系统说明 | 系统介绍 | 数据规模、公开数据来源、Spark 清洗流程、数仓分层表清单、更新时间与更新日志 |

> 数据整合 / 数据质量两页所有数字均来自治理脚本真实输出（`data/processed/data_quality_report.md`），由 `build_spa.sh` 注入快照 `overviews.data_quality` 随离线大屏分发，非界面演示数据。
